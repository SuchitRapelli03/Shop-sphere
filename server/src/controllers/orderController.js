...
export async function createOrder(req, res) {
  ...
  const mixedStore = cart.items.some(
    (item) => item.productId.storeId.toString() !== firstStoreId.toString()
  );
  if (mixedStore) {
    return res.status(400).json({
      message: "This MVP supports one store per order. Split your cart by store."
    });
  }
...
export async function vendorOrders(req, res) {
  const orders = await Order.find({ vendorId: req.user._id }).sort({ createdAt: -1 });
  res.json({ orders });
}
